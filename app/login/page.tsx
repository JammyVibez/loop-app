...
export default function LoginPage() {
  ...
  useEffect(() => {
    ...
    const messages: Record<string, string> = {
      auth_error: "We could not complete authentication. Please try again.",
      profile_error: "Your account was created, but the profile could not be loaded.",
      server_error: "Authentication is temporarily unavailable. Please try again.",
    }

    setError(messages[errorCode] || "Authentication failed. Please try again.")
  }, [searchParams])
    ...
    <div className="relative min-h-screen overflow-hidden bg-[#030712] text-slate-100 flex items-center justify-center p-4">
      ...
      <Card className="relative z-10 w-full max-w-md landing-card-3d border-white/10 bg-[#0a1020]/90 text-slate-100 shadow-2xl shadow-violet-950/30 backdrop-blur-xl">
        <CardHeader className="space-y-1 text-center">
          ...
          <CardDescription>Sign in to your Loop account</CardDescription>
        </CardHeader>
        <form onSubmit={handleSubmit}>
          ...
          <CardFooter className="flex flex-col space-y-4">
            ...
            <div className="flex flex-col space-y-2 text-center text-sm text-slate-400">
              <Link href="/reset-password" className="text-purple-600 hover:text-purple-700">
